...
ATAC_Seq_DBS_Analysis <- function(Sample_Sheet_csv_file, celltype, group1, group2, DE_Method = DBA_EDGER, Normalization_Method = DBA_NORM_TMM, usePval = F, pval_th = 0.01, FC_th = -log(0.7), Capped_th = 5, outputdir = "./", dba_counts_RDS_File = "") {
  ...
  png(paste0(outputdir,celltype,'_peak_counts_correlation_heatmap_',DE_Method,'_',Normalization_Method,'.png'), units="in", width=7, heigh=7, res=800)
  dba.plotHeatmap(dba_Counts_norm_contrast_analyzed, main = "Correlation Plot for CD4 Samples")
  dev.off()
  
  png(paste0(outputdir,celltype,'_peak_counts_PCA_',DE_Method,'_',Normalization_Method,'.png'), units="in", width=7, heigh=7, res=800)
  ...
  png(paste0(outputdir,celltype,'_Volcano_Plot_',DE_Method,'_',Normalization_Method,'.png'), units="in", width=7, heigh=7, res=800)
  print(volcano_plot)
  dev.off()
...
# Load the sample sheet
CD4_samples <- read.csv("Sample_Sheet_CD4.csv")

# Initialize the dba object with the sample sheet
CD4 <- dba(sampleSheet=CD4_samples) 
CD4_blklst <- dba.blacklist(CD4) 
CD4_Counts <- dba.count(CD4_blklst) #bParallel = TRUE, score=DBA_SCORE_READS, summits=75) 
saveRDS(CD4_Counts,file = "CD4_Counts.RDS")
```

```{r}
CD4_Counts_norm <- dba.normalize(CD4_Counts, method = DBA_EDGER, normalize = DBA_NORM_TMM)# method = DBA_DESEQ2, normalize = DBA_NORM_RLE)  
CD4_Counts_norm_contrast <- dba.contrast(CD4_Counts_norm, contrast = c("Condition", "good", "poor"))
CD4_Counts_norm_contrast_analyzed <- dba.analyze(CD4_Counts_norm_contrast, method = DBA_EDGER)#DBA_DESEQ2)
#CD4_Counts <- dba.count(CD4, bParallel = TRUE, score=DBA_SCORE_READS)

png('./CD4_counts_correlation_heatmap.png', units="in", width=7, heigh=7, res=800)
dba.plotHeatmap(CD4_Counts_norm_contrast_analyzed, main = "Correlation Plot for CD4 Samples")
#plot(CD4_Counts, main="Correlation Plot for CD4 Samples")
dev.off()

png('./CD4_counts_PCA.png', units="in", width=7, heigh=7, res=800)
dba.plotPCA(CD4_Counts_norm_contrast_analyzed,DBA_CONDITION, label=DBA_ID) #DBA_CONDITION
dev.off()

#plot(CD4_Counts_norm_contrast_analyzed, contrast= 1)


##NOTICE!!! USING PVAL INSTEAD OF FDR:
CD4_Counts_norm_contrast_analyzed.DB <- dba.report(CD4_Counts_norm_contrast_analyzed,th=0.05, contrast = 1,bUsePval=T, method=DBA_EDGER, bCounts=TRUE) #DBA_DESEQ

write.csv(as.data.frame(CD4_Counts_norm_contrast_analyzed.DB),"./CD4_Counts_norm_contrast_analyzed_DBS_EDGER_TMM.csv")

venn_Gain_Loss<-dba.plotVenn(CD4_Counts_norm_contrast_analyzed, contrast = 1, method = DBA_DESEQ2,bUsePval =TRUE, bDB=TRUE,bGain=TRUE, bLoss=TRUE, bAll=FALSE, th=0.05,label1 = "Good", label2 = "Poor")

dba.plotVolcano(CD4_Counts_norm_contrast_analyzed,bUsePval = T,method=DBA_EDGER,fold = -log2(0.7),bLabels=F)

pvals <- dba.plotBox(CD4_Counts_norm_contrast_analyzed,contrast = 1,bUsePval = T ,method = DBA_DESEQ2)

#hmap <- colorRampPalette(c("red", "black", "green"))(n = 13)
#library(RColorBrewer)
readscores <- dba.plotHeatmap(CD4_Counts_norm_contrast_analyzed, contrast=1,bUsePval = T ,method = DBA_DESEQ2, correlations=FALSE,scale="row", colScheme = colorRampPalette(brewer.pal(10,"RdBu"))(256))
...
```{r}
###########                                   Volcano                     ############
dba_df <- as.data.frame(dba.report(CD4_Counts_norm_contrast_analyzed,th=1, contrast = 1,bUsePval=T, method=DBA_EDGER, bCounts=TRUE))

dba_df <- add_gene_names(dba_df, CD4_Counts_norm_contrast_analyzed)

# Define significance thresholds
pval_threshold <- 0.05
fold_change_threshold <- -log2(0.7)

# Create a new column to classify points based on significance
dba_df$Significance <- with(dba_df, ifelse(p.value < pval_threshold & abs(Fold) > fold_change_threshold, "Significant", "Not Significant"))

# Calculate -log10 p-value
dba_df$negLog10Pval <- -log10(dba_df$p.value) # or use P.value if you prefer p-values over FDR


# Cap -log10(p-value) at 5 for plotting
dba_df$negLog10Pval_capped <- ifelse(dba_df$negLog10Pval > 5, 5, dba_df$negLog10Pval)
# Create a new column to classify points that are capped
dba_df$Capped <- ifelse(dba_df$negLog10Pval > 5, "Capped", "Not Capped")

# Identify top significant genes
top_genes <- dba_df[dba_df$Significance == "Significant",]
top_genes <- top_genes[order(top_genes$p.value), ] # Order by FDR
top_genes <- head(top_genes, 10) # Select top 10 genes
...
library(ggrepel)
# Create the volcano plot using ggplot2
volcano_plot <- ggplot(dba_df, aes(x = Fold, y = negLog10Pval_capped, color = Significance, shape = Capped)) +
  geom_point(alpha = 0.5) +
  theme_minimal() +
  scale_color_manual(values = c("Significant" = "red", "Not Significant" = "grey")) +
  scale_shape_manual(values = c("Capped" = 17, "Not Capped" = 16)) + # Different shapes for capped points
  ...
  geom_vline(xintercept = c(-fold_change_threshold, fold_change_threshold), col = "blue", linetype = "dashed") +
  geom_text_repel(data = top_genes, aes(label = SYMBOL), size = 3) +
  labs(title = "Volcano Plot", x = "Log2 Fold Change", y = "-log10(Pvalue)", color = "Gene Significance", shape = "Capped Values") +
  ylim(0, 5) +
  annotate("text", x = max(dba_df$Fold), y = 4.5, label = "Red: Significant genes\n(pvalue < 0.05 and abs(log2Fold) > log2(0.7))", hjust = 1, size = 3)

# Display the plot
print(volcano_plot)

write.csv(dba_df, "CD4_Counts_EDGER_TMMnorm_contrast_analyzed.csv")
...
tf_data <- read.table("Homo_sapiens_TF.txt", header = TRUE, sep = "\t")
dba_df_tfs <- subset(dba_df, dba_df$SYMBOL %in% tf_data$Symbol)
write.csv(dba_df_tfs[order(dba_df_tfs$p.value,decreasing = FALSE),], "TFS_CD4_Counts_EDGER_TMMnorm_contrast_analyzed.csv")
...
dba_df_sig <- subset(dba_df, Significance == "Significant")

# Filter rows with greatest absolute Foldchange for each gene
dba_df_sig_NoDups <- dba_df_sig %>%
  group_by(SYMBOL) %>%
  slice(which.max(abs(Fold))) %>%
  ungroup()

#df_ForGSEA <- subset(dba_df_sig_NoDups, select = c(Fold,p.value,SYMBOL))
df_ForGSEA <- subset(dba_df, select = c(Fold,p.value,SYMBOL))

 

#msigdb_c7 <- msigdbr(species = "Homo sapiens", category = "C7")
#c7_list <- split(msigdb_c7$gene_symbol, msigdb_c7$gs_name)
...
fgseaRes_CD8 <- subset(fgseaRes, grepl("CD8", pathway))
fgseaRes_CD8 <- fgseaRes_CD8[order(fgseaRes_CD8$NES,decreasing = TRUE),]
fgseaRes_CD8_csv <- fgseaRes_CD8
fgseaRes_CD8_csv$leadingEdge <- sapply(fgseaRes_CD8_csv$leadingEdge, paste, collapse = ", ")

# Optionally, save these subsets to CSV files
write.csv(fgseaRes_CD4_csv, file = "fgseaRes_CD4_C7gmt_sig.csv", row.names = FALSE)
write.csv(fgseaRes_CD8_csv, file = "fgseaRes_CD8_C7gmt_sig.csv", row.names = FALSE)
...
topPathwaysUp <- fgseaRes[NES > 0][order(pval)][1:10,]
topPathwaysDown <- fgseaRes[NES < 0][order(pval)][1:10,]
topPathways <- rbind(topPathwaysUp, na.omit(topPathwaysDown))
...
header = paste0("Top ",nrow(topPathways)," (Total pathways: Up=", nrow(topPathwaysUp),", Down=",  nrow(topPathwaysDown), ")")
...
header = paste0("Top ",nrow(fgseaRes_CD4_top30)," (Total pathways: Up=", total_up,", Down=",  total_down, ")")
...
```{r}
#Removing not important pathways:
fgseaRes_CD8_filt_NonImportant <- fgseaRes_CD8[-which(fgseaRes_CD8$pathway %in% fgseaRes_CD8[c(3,7,9,14,15,16,20,22),]$pathway),]

fgseaRes_CD8_top30 = fgseaRes_CD8_filt_NonImportant[1:30,]
fgseaRes_CD8_top30$Enrichment = ifelse(fgseaRes_CD8_top30$NES > 0, "Up-regulated", "Down-regulated")

fgseaRes_CD8_top30 = rbind(head(fgseaRes_CD8_top30, n = nrow(subset(fgseaRes_CD8_top30, NES>0))),
                tail(fgseaRes_CD8_top30, n = nrow(subset(fgseaRes_CD8_top30, NES<0)) ))
total_up = sum(fgseaRes_CD8_top30$Enrichment == "Up-regulated")
total_down = sum(fgseaRes_CD8_top30$Enrichment == "Down-regulated")
header = paste0("Top ",nrow(fgseaRes_CD8_top30)," (Total pathways: Up=", total_up,", Down=",  total_down, ")")

colos = setNames(c("firebrick2", "dodgerblue2"),
           c("Up-regulated", "Down-regulated"))

ggplot(fgseaRes_CD8_top30, aes(reorder(pathway, NES), NES)) +
geom_point( aes(fill = Enrichment, size = size), shape=21) +
scale_fill_manual(values = colos ) +
scale_size_continuous(range = c(2,10)) +
geom_hline(yintercept = 0) +
...
bamfiles <- list.files(path = "D:\\Kfir\\ATAC-Seq\\Bwa-mem_Macs2_Output", pattern = "*.bam$", full.names = TRUE)
...
shifted.bamFile="D:/Kfir/ATAC-Seq/Bwa-mem_Macs2_Output/G08CD4_S145927_NoMT_subset.blacklist-filtered.shifted.sorted.bam"
...
# Read the gene annotation data (replace 'annotations.gtf' with your file)
#annotations <- rtracklayer::import('gencode.v46.annotation.gtf')

# Convert to a data frame with relevant columns
...
data_matrix <- dba.plotHeatmap(CD4_Counts_norm_contrast_analyzed, contrast=1, bUsePval=TRUE,
                               method=DBA_DESEQ2, correlations=FALSE, scale="row")
data_matrix <- as.data.frame(data_matrix)

merged_data <- add_gene_names(data.matrix, CD4_Counts_norm_contrast_analyzed)
...
pheatmap_CD4 <- pheatmap::pheatmap(as.matrix(averaged_data %>% `rownames<-`( NULL )), color = heatmap_colors,
        show_row_names = F,cluster_rows =T, treeheight_row =0, show_column_names = F, scale="row", row_names_gp = gpar(fontsize = 0.5),annotation_col = heatmap_annotation_df, annotation_colors = heatmap_annotation_colors) 
pheatmap_CD4
...
png('./CD4_DBA_PlotProfile.png', units="in", width=7, heigh=7, res=800)
...
profiles2 <- dba.plotProfile(CD4_Counts_norm_contrast_analyzed, sites=granges_list, contrast=1, 
                ...
                labels=c("Good", "Poor"))
png('./CD4_DBA_PlotProfile2.png', units="in", width=7, heigh=7, res=800)
dba.plotProfile(profiles)
dev.off()
```
